...
public class TopPanel extends JPanel implements Scrollable, ComponentListener
{
	...
	private JLabel jlblOverallFitScoreLabel = null;
	public JLabel jlblOverallFitScore = null;
	private JPanel jpanelExtractedIonChromatogram = null;
	private JPanel jpanelChoosePeak = null;
	/**
	...
	public void componentResized(ComponentEvent arg0) {
		...
		if (arg0.getComponent() == this)
		{
			...
			this.jlblOverallFitScoreLabel.setLocation(jlblOverallFitScoreLabel.getX(), jPanelStandards.getHeight() - 4 - jlblOverallFitScoreLabel.getHeight() - 12);
			this.jlblOverallFitScore.setLocation(jlblOverallFitScore.getX(), jlblOverallFitScoreLabel.getY());
			this.jScrollPaneStandards.setSize(jPanelStandards.getWidth() - 24, jlblOverallFitScoreLabel.getY() - jScrollPaneStandards.getY() - 4);
			...
			this.jpanelChoosePeak.setSize((jPanelPeakPick.getWidth() * 325) / 733, jPanelPeakPick.getHeight() - 86);

			// Scale table
			this.jScrollPane1.setLocation(jpanelChoosePeak.getX() + jpanelChoosePeak.getWidth() + 4, jpanelChoosePeak.getY());
			this.jScrollPane1.setSize(jPanelPeakPick.getWidth() - jScrollPane1.getX() - 12, jpanelChoosePeak.getHeight() - 84);
			...
			this.jLabel.setLocation(jScrollPane1.getX() + 2, jScrollPane1.getY() + jScrollPane1.getHeight() + 8);
			this.jLabel.setSize((jScrollPane1.getWidth() / 2) - 2, jLabel.getHeight());
			this.jLabel1.setLocation(jScrollPane1.getX() + 2, jLabel.getY() + jLabel.getHeight() + 8);
			...
			this.jchkNotHere.setLocation(jLabel1.getX(), jLabel1.getY() + jLabel1.getHeight() + 8);
	...
	private JScrollPane getJScrollPaneStandards() {
		if (jScrollPaneStandards == null) {
			...
			jScrollPaneStandards.setBounds(new Rectangle(12, 28, 285, 537));
	...
	private JPanel getJPanelEICTotal() {
		if (jPanelEICTotal == null) {
			...
			jlblExpectedPeakWidth.setBounds(new Rectangle(540, 308, 181, 16));
			...
			jlblExpectedRetentionTime.setBounds(new Rectangle(540, 284, 181, 16));
			jlblExpectedRetentionTime.setText("0 min");
			jLabel1 = new JLabel();
			jLabel1.setText("Expected peak width (FWHM):");
			jLabel1.setBounds(new Rectangle(344, 308, 193, 16));
			jLabel1.setFont(new Font("Dialog", Font.PLAIN, 12));
			jLabel = new JLabel();
			jLabel.setFont(new Font("Dialog", Font.PLAIN, 12));
			jLabel.setBounds(new Rectangle(344, 284, 193, 16));
	...
	private JPanel getJPanelStandards() {
		if (jPanelStandards == null) {
			jlblOverallFitScore = new JLabel();
			jlblOverallFitScore.setBounds(new Rectangle(160, 572, 137, 16));
			jlblOverallFitScore.setText("0");
			jlblOverallFitScore.setFont(new Font("Dialog", Font.PLAIN, 12));
			jlblOverallFitScoreLabel = new JLabel();
			jlblOverallFitScoreLabel.setBounds(new Rectangle(12, 572, 141, 16));
			jlblOverallFitScoreLabel.setText("Overall fit score:");
			jlblOverallFitScoreLabel.setFont(new Font("Dialog", Font.PLAIN, 12));
			jPanelStandards = new JPanel();
			jPanelStandards.setLayout(null);
			jPanelStandards.setBounds(new Rectangle(4, 4, 309, 641));
			jPanelStandards.setBorder(BorderFactory.createTitledBorder(null, "Standard Compounds", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, new Font("Dialog", Font.BOLD, 12), new Color(51, 51, 51)));
			jPanelStandards.add(getJScrollPaneStandards(), null);
			jPanelStandards.add(jlblOverallFitScoreLabel, null);
			jPanelStandards.add(jlblOverallFitScore, null);
		}
	...
	private JPanel getJPanelPeakPick() {
		if (jPanelPeakPick == null) {
			...
			jPanelPeakPick.add(getJpanelChoosePeak(), null);
		}
	...
	private JButton getJbtnNext() {
		if (jbtnNext == null) {
			...
			jbtnNext.setBounds(new Rectangle(536, 360, 185, 41));
			jbtnNext.setIcon(new ImageIcon(getClass().getResource("/boswell/peakfindergc/images/forward.png")));
	...
	private JButton getJbtnPrevious() {
		if (jbtnPrevious == null) {
			...
			jbtnPrevious.setBounds(new Rectangle(340, 360, 185, 41));
			jbtnPrevious.setIcon(new ImageIcon(getClass().getResource("/boswell/peakfindergc/images/back.png")));
	...
	private JCheckBox getJchkNotHere() {
		if (jchkNotHere == null) {
			...
			jchkNotHere.setBounds(new Rectangle(344, 332, 377, 21));
	...
	private JPanel getJpanelExtractedIonChromatogram() {
		if (jpanelExtractedIonChromatogram == null) {
			...
			jpanelExtractedIonChromatogram.setBounds(new Rectangle(12, 28, 709, 181));
	...
	/**
	 * This method initializes jpanelChoosePeak	
	 * 	
	 * @return javax.swing.JPanel	
	 */
	private JPanel getJpanelChoosePeak() {
		if (jpanelChoosePeak == null) {
			GridLayout gridLayout1 = new GridLayout();
			gridLayout1.setRows(1);
			jpanelChoosePeak = new JPanel();
			jpanelChoosePeak.setLayout(gridLayout1);
			jpanelChoosePeak.setBounds(new Rectangle(12, 28, 325, 325));
			jpanelChoosePeak.setBorder(BorderFactory.createLineBorder(Color.gray, 1));
			jpanelChoosePeak.add(m_PeakEICGraphControl, null);
		}
		return jpanelChoosePeak;
	}
