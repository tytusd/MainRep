...
class InterpolationFunction implements Serializable
{
	...
	private void writeObject(ObjectOutputStream out) throws IOException
	{
		...
		out.writeObject(dInterpolationParameters);
		out.writeObject(dRanges);
		out.writeBoolean(bLinear);
		out.writeObject(lifTwoPoint);
	}
	...
	private void readObject(ObjectInputStream in) throws IOException, ClassNotFoundException
	{
		...
		if (lVersion >= 1)
		{
			dInterpolationParameters = (double[][])in.readObject();
			dRanges = (double[])in.readObject();
			bLinear = (boolean)in.readBoolean();
			lifTwoPoint = (LinearInterpolationFunction)in.readObject();
		}
	}
