...
public class MeasureYourGradientApplet extends JApplet implements ActionListener, KeyListener, FocusListener, ListSelectionListener, AutoScaleListener, TableModelListener, ClipboardOwner
{
    ...
    public LinearInterpolationFunction m_InitialInterpolatedDeadTimeProfile;
...
*/
    ...
    public void nextStepButtonPressed()
    {
    	...
    	{
    		String message = "Please enter your measured instrument dead time in the \"Dead time without column\" box. "
    						+ "Not entering a value in this box can cause significant error in your back-calculated profiles unless your instrument dead time is exceedingly small."
    						+ "<BR><BR>"
    						+ "Do you wish to continue anyway?";
    		
    		int retVal = MsgDlg.question(null, message, "Measure Your Gradient", JOptionPane.YES_NO_OPTION);
    	...
    	for (int i = 0; i < contentPane.tmMeasuredRetentionTimes.getRowCount(); i++)
    	{
    		...
    		Object[] newRow = {strCompoundName, formatter.format(dMeasuredRetentionTime), null, null};
    ...
    public void placeHandles()
    {
	    ...
	    for (int i = 0; i < iNumFlowDataPoints; i++)
		{
	    	...
	    	//m_dDeadTimeDifferenceArray[i][1] = 0;
...
//			Globals.hbMainHelpBroker.setDisplayed(true);
	    ...
	    else if (strActionCommand == "Preloaded Values")
	    {
    		...
    		contentPane.jtxtIDT.setText(((Double)Globals.dGradientPrograms[dlgPreloadedValues.m_iCondition][0][3]).toString());
	    ...
	    else if (strActionCommand == "Find retention times automatically...")
	    {
	    	...
	    	peakFinderSettingsDialog.setIsocraticData(Globals.StandardIsocraticDataArray);
	    	peakFinderSettingsDialog.setStandardCompoundNames(Globals.CompoundNameArray);
	    	peakFinderSettingsDialog.setStandardCompoundMZData(Globals.CompoundsMZArray);
	...
	public void performValidations()
	{
        ...
        this.m_dInitialDeadTimeArray = new double[Globals.dDeadTimeArray.length][2];
        ...
        for (int i = 0; i < Globals.dDeadTimeArray.length; i++)
        {
        	...
        	this.m_dInitialDeadTimeArray[i][0] = Globals.dDeadTimeArray[i][0];
        	this.m_dInitialDeadTimeArray[i][1] = (dNewDeadVol / this.m_dFlowRate) * 60;
        }
        
		this.m_InitialInterpolatedDeadTimeProfile = new LinearInterpolationFunction(this.m_dInitialDeadTimeArray);
...
/*			if (event.getAutoScaleXState() == true)
        ...
        public void done() 
        {
        	if (!this.isCancelled())
        	{
	        	...
	        	if (dRatio > 8 && !m_bNoFullBackcalculation)
	        	{
	            	...
	            	String strMessage = "Please double-check the retention time you reported for " + Globals.CompoundNameArray[(Integer)m_vectCalCompounds.get(iCompoundWithMaxError)[0]] + ". It does not seem to be correct."
	            		+ "<br><br>"
	            		+ "To correct the retention time you entered, click \"Previous Step\", change the retention time, and re-run the back-calculation.";
	        		MsgDlg.alert(null, strMessage, "Measure Your Gradient");
	        	...
	        	{
	        		// Report a problem with the retention time for the compound
	            	String strMessage = "Your variance is high. This usually indicates that more than one of the retention times you entered are incorrect. Please double-check all of the retention times you entered."
            		+ "<br><br>"
            		+ "To correct a retention time you entered, click \"Previous Step\", change the retention time, and re-run the back-calculation.";
	            	MsgDlg.alert(null, strMessage, "Measure Your Gradient");
	...
	public void backCalculate(Task task, boolean bDeadTimeProfileFirst)
	{
		...
		boolean bBackCalculateMixingVolume = true;
		boolean bBackCalculateDeadTimeProfileOffset = true;
		...
		int iNumCompoundsToUseForDeadTimeOffset = m_vectCalCompounds.size(); // 2;
		...
		/*//
		{
		m_dMixingVolume = 0.001;
		m_dNonMixingVolume = 0.001;
		calculateSimpleGradient();
		dRetentionError = calcRetentionError(m_dtstep, m_vectCalCompounds.size(), true);
		...
		while (true)
		{
			...
			if (bBackCalculateSimpleGradientProfile)
			{
				...
				contentPane2.jlblMixingVolume.setText(formatter3.format(this.m_InterpolatedMixingVolume.getAt(this.m_dIdealGradientProfileArray[0][1])) + " mL");
				contentPane2.jlblNonMixingVolume.setText(formatter3.format(this.m_InterpolatedNonMixingVolume.getAt(this.m_dIdealGradientProfileArray[0][1])) + " mL");
				
				double dAverageTotalGradientDelay = 0;
				for (int i = 0; i < m_dNonMixingVolumeArray.length; i++)
				{
					dAverageTotalGradientDelay += (this.m_dNonMixingVolumeArray[i][1] + this.m_dMixingVolumeArray[i][1]) / (double)m_dNonMixingVolumeArray.length;
				}
				contentPane2.jlblTotalGradientDelayVolume.setText(formatter3.format(dAverageTotalGradientDelay) + " mL");
			...
			if (!bSkipTerminationTest)
			{
				...
				if (iPhase == 1)
				{
					if (dPercentImprovement < .1 && dPercentImprovement >= 0)
					{
						...
						bBackCalculateMixingVolume = true;
				...
				if (iPhase == 2)
				{
					if (dPercentImprovement < .1 && dPercentImprovement >= 0)
					{
			    		...
			    		String message = "Your gradient delay was successfully back-calculated."
			    			+ "<BR><BR>"
			    			+ "To back-calculate your gradient with more precision, you may continue the optimization. The process will take several minutes."
			    			+ "<BR><BR>"
			    			+ "Do you want to finish optimizing your gradient?";
    		
			    		int retVal = MsgDlg.question(this, message, "Measure Your Gradient", JOptionPane.YES_NO_OPTION);
						
						if (retVal == JOptionPane.NO_OPTION)
						{
							m_bNoFullBackcalculation = true;
							break;
						}
